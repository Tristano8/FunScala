...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (for {
      (char, occ) <- w.toLowerCase().groupBy(char => char)
    } yield (char, occ.length)).toList.sortWith(_._1 < _._1)
  }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs => {
        (for {
          i <- combinations(xs)
          j <- 1 to x._2
        } yield (x._1, j) :: i) ::: combinations(xs)
    }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def occurrencesAnagrams(occurrences: Occurrences): List[Sentence] = occurrences match {
      case Nil => List(List())
      case _ => for {
        i <- combinations(occurrences)
        j <- dictionaryByOccurrences(i)
        restOfSentence <- occurrencesAnagrams(subtract(occurrences, i))
      } yield j :: restOfSentence
    }
